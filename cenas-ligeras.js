...
const PROFILES = {
  improvisacion: {
    ...
    patron: 'Tu patrón actual: demasiadas decisiones en el momento en que tienes menos energía para tomarlas.',
    influye: 'Llegas a la tarde o la noche sin una opción clara, y eso hace que improvisar parezca más fácil.',
    estrategia: 'Tu mejor estrategia: dejar algunas decisiones preparadas antes del momento difícil.',
    paso: 'Elige 2 o 3 cenas esta semana y deja al menos dos porciones listas antes de empezar.',
  ...
  hambre: {
    ...
    patron: 'Tu patrón actual: llegar a la tarde o noche con hambre acumulada o con una cena poco satisfactoria.',
    influye: 'Cuando el hambre es intensa, las decisiones rápidas ganan. Y la sensación de que falta algo hace todo más difícil.',
    estrategia: 'Tu mejor estrategia: organizar cenas ligeras con volumen, sabor y porciones que realmente te dejen satisfecha.',
    paso: 'Empieza con recetas de mayor volumen y organiza al menos dos porciones para los días de más hambre.',
  ...
  rutina: {
    ...
    patron: 'Tu patrón actual: depender de soluciones que funcionan los primeros días, pero tarde o temprano se vuelven insostenibles.',
    influye: 'Cuando una rutina exige mucho esfuerzo cada vez, mantenerla depende de motivación — y la motivación fluctúa.',
    estrategia: 'Tu mejor estrategia: una estructura sencilla, con variedad suficiente y que puedas repetir incluso en una semana difícil.',
    paso: 'Empieza con una sola semana organizada, en lugar de intentar cambiar todo de una vez.',
    bridge: 'Para ti, la clave no es más esfuerzo — es un sistema que no dependa de estar motivada cada día para funcionar.'
  },
  tiempo: {
    title: 'La cena compite con todo lo demás del día',
    patron: 'Tu patrón actual: demasiadas decisiones y tareas para resolver la cena cada noche.',
    influye: 'Pensar qué comprar, qué cocinar, cuánto preparar y cómo adaptarlo para todos convierte la cena en una tarea pesada.',
    estrategia: 'Tu mejor estrategia: preparar varias porciones en una sola sesión para reducir el trabajo diario.',
    paso: 'Usa una lista única y organiza parte de tus cenas antes de comenzar la semana.',
    bridge: 'Con tu ritmo, preparar una vez para varias noches es la solución más directa que existe para recuperar tus tardes.'
  },
  low: {
    title: 'Tu rutina ya tiene una buena base',
    patron: 'Tu rutina tiene una base más sólida que la mayoría.',
    influye: 'Tu principal oportunidad está en reducir las decisiones durante los días más ocupados.',
    estrategia: 'Tu mejor estrategia: tener opciones listas para los momentos en que tu rutina cambia.',
    paso: 'Deja dos cenas organizadas antes de comenzar cada semana.',
    bridge: 'Tu punto de partida es mejor que el de la mayoría — lo que necesitas es un sistema claro para los días más difíciles.'
  }
};
...
// ── Objeción blocks ──────────────────────────────────────
const OBJ = {
  complicada:    { q:'¿Te preocupa que sea complicado?', a:'El método fue creado para uso doméstico. Las recetas tienen instrucciones paso a paso y la Preparación Semanal te muestra cómo organizar varias porciones en una sola sesión, sin técnicas especiales.' },
  cocinar_diario:{ q:'¿No tienes tiempo para cocinar todos los días?', a:'El sistema funciona exactamente al revés: cocinas una o dos veces por semana y resuelves varias noches. La Preparación Semanal te enseña cómo.' },
  hambre:        { q:'¿Tienes miedo de quedarte con hambre?', a:'Las recetas incluyen orientación de porciones. El objetivo no es una porción mínima, sino una cena ligera, con volumen y sabor, que realmente te deje satisfecha.' },
  ingredientes:  { q:'¿Los ingredientes son difíciles de encontrar?', a:'La Guía Latinoamericana reúne nombres regionales y alternativas para México, Colombia, Estados Unidos y otros mercados. Una receta no debe depender de un solo ingrediente.' },
  familia:       { q:'¿Tu familia no va a querer comer lo mismo?', a:'El Bono 5 (Versiones para Toda la Familia) te muestra cómo adaptar o complementar la misma base sin tener que empezar otra comida desde cero.' },
  motivacion:    { q:'¿Empiezas con energía pero no logras mantenerlo?', a:'Por eso recibes un Plan de 21 Días y una Preparación Semanal. Empiezas con una estructura pequeña y repetible — fácil de repetir incluso en una semana difícil.' }
};
...
function personalizeTSL() {
  ...
  // Hero profile tag (dynamic, bridges quiz → TSL)
  const RESULT_LABELS = {
    ...
    low:           'TU DIÁGNÓSTICO LISTO'
  };
  const heroTag = document.getElementById('hero-profile-line');
  if (heroTag) {
    const label = RESULT_LABELS[S.profile] || RESULT_LABELS.low;
    heroTag.textContent = 'TU RESULTADO: ' + label;
    heroTag.style.display = 'inline-block';
  }

  // El Ciclo — personalized first sentence
  const CICLO_INTRO = {
...
function trackEvent(name, data) {
  try { if(typeof fbq==='function') fbq('trackCustom', name, data||{}); } catch(e){}
